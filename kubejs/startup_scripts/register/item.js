...
$Rarity.create("COSMIC", $ChatFormatting.RED)
$Rarity.create("ROSARIUM", $ChatFormatting.DARK_PURPLE)
$Rarity.create("AURANTIUM", $ChatFormatting.GOLD);
$Rarity.create("AMETHYSTINE", $ChatFormatting.LIGHT_PURPLE);

StartupEvents.registry("item", event => {
    event.create("materialfactory:rainbow_slimeball", "basic")
    ...
    event
        .create('materialfactory:annulus')
        .rarity('advanced')
        .unstackable()
...
//并行插件
let $AugmentItem = Java.loadClass('cofh.thermal.lib.common.item.AugmentItem')
let $Item$Properties = Java.loadClass("net.minecraft.world.item.Item$Properties")
let $AugmentDataHelper = Java.loadClass('cofh.core.util.helpers.AugmentDataHelper')
let $NBTTags = Java.loadClass('cofh.lib.util.constants.NBTTags');
let $TePaConstants = Java.loadClass('com.c2h6s.thermal_parallel.util.TePaConstants');
StartupEvents.registry("item", event => {
    event.createCustom("thermal_parallel:argument_parallel_4", () => {
        let properties = new $Item$Properties().stacksTo(64).rarity("uncommon");
        let augmentData = $AugmentDataHelper.builder()
            .type($NBTTags.TAG_AUGMENT_TYPE_MACHINE)
            .mod($TePaConstants.TAG_MACHINE_PARALLEL, 64)
            .build();

        return new $AugmentItem(properties, augmentData);
    });
    event.createCustom("thermal_parallel:argument_parallel_5", () => {
        let properties = new $Item$Properties().stacksTo(64).rarity("epic");
        let augmentData = $AugmentDataHelper.builder()
            .type($NBTTags.TAG_AUGMENT_TYPE_MACHINE)
            .mod($TePaConstants.TAG_MACHINE_PARALLEL, 256)
            .build();

        return new $AugmentItem(properties, augmentData);
    });
    event.createCustom("thermal_parallel:argument_parallel_6", () => {
        let properties = new $Item$Properties().stacksTo(64).rarity("supreme");
        let augmentData = $AugmentDataHelper.builder()
            .type($NBTTags.TAG_AUGMENT_TYPE_MACHINE)
            .mod($TePaConstants.TAG_MACHINE_PARALLEL, 1024)
            .build();
